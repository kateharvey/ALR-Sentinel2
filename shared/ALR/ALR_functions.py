...
# It returns an image with the response band centred to a mean 0, and the other bands in the image standardized to a mean 0 and 
# standard deviation 1. This preprocessing is necessary for the LARs algorithm
...
def ee_LARS(image, response_band, num_nonzero_coefficients, num_samples):
    ...
    # Retrieve the list of features in the image by getting all band names and removing response band name from list
    bandList = image.bandNames()
    featureList = bandList.remove(response_band)
